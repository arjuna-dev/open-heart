...
In CSS 3D mode, every wheel or trackpad delta is applied to the pose in the same input event, so a ten-pixel gesture produces a corresponding ten-pixel-equivalent movement instead of waiting for a queued animation. There is no scroll-behavior queue and no artificial opening pause. Each active turn is clamped to one adjacent face: ten percent of the face span commits the turn, remaining momentum is consumed while that turn is moving, and the next delta can begin the next face as soon as the landing completes. A smaller nudge settles back. This prevents a single event from skipping two pages while keeping continued scrolling responsive. Committed turns complete as one smooth 720-millisecond vertical rotation. Both adjoining faces remain visible and interactive while the turn is in progress, and a landed face leaves the transformed 3D scene and becomes a true flat reading surface. The landed scroll anchor is held during pointer, focus, and text-selection gestures so browser focus scrolling cannot rotate the scene underneath a click. If a face is taller than the viewport, as the full menu can be, wheel input scrolls that face first and turns to the next face only after reaching its edge. The fixed **Turn the page** pager remains available in 3D and centered reference modes; it stays hidden in the default simple-scroll view.
