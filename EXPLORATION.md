...
The menu category carousel stays on autoplay while the pointer is over it. Studio controls expose three GSAP experiments: slide the old category out to the left while the new category enters from the right, fade between categories, or switch animation off. The active category is marked in the tab rail, live counter, and menu state.
