...
The developer menu includes two sparse composition directions and five web ink recipes. `Centered pair SMALL` is the tighter working direction, with an exact 300-pixel copy column and exact 300-pixel image column shared by every non-menu face. Pages differ in content, not in their core pair geometry. `Centered pair / reference` gives the group more room while preserving the supplied sparse study: large empty fields, a small left copy block, a right-hand image, and fixed studio chrome. The size difference remains visible on mobile, where SMALL uses a 300-pixel copy and 240-pixel image while the reference composition can use a 330-pixel copy and 280-pixel image. The former full-editorial mode and its unused CSS have been removed. The baseline Open Heart wordmark is used in both modes. The inverted `Brick red + cyan` recipe uses brick red for paper-face type and a softened brick field, with cyan taking the final field.

The menu presentation dropdown can switch among the image-led category carousel, an ultra-minimal menu study, and a centered-pair SMALL study. The ultra-minimal study uses deliberately tiny list typography, consolidates the tabs into Benedict + sandwiches, entrees + sauces, and drinks + desserts, and renders each group as two headed category columns plus one poster column. The centered-pair SMALL option places the current category inside the exact shared constraints, with one text column and one image column.
...
The CSS 3D experiment uses one consistent vertical turn for all five transitions. Each wheel delta updates the pose in the same event, preserving direct distance-to-motion feedback. There is no browser smooth-scroll queue. One active turn is clamped to one adjacent face, and momentum after the ten-percent commit threshold is consumed while that turn is moving. As soon as the landing completes, the next wheel delta can begin the next face, so continued scrolling does not get trapped. Smaller nudges settle back to the current face, while committed turns complete over 720 milliseconds with gentle sine easing so the shared edge and shading can be read. The animated face shadows use promoted, backface-hidden paint layers to reduce compositing flicker, and Studio can disable those layers entirely for a geometry-only comparison. A landed page is removed from the transformed 3D context and restored as a flat 2D reading surface, which keeps text selection and controls reliable. Wheel input scrolls overflow inside the active face first, including the full menu, then advances to the next face at the boundary. Transformed inactive faces remain inert and the fixed page pager remains available.

The menu category carousel waits 5.6 seconds between changes and provides a persistent Pause or Play button in every presentation. Following the WAI carousel pattern, initial autoplay pauses on hover and stops when keyboard focus enters the carousel; focus-stopped rotation restarts only when the visitor explicitly selects Play. Selecting a category, previous or next control, or menu item also pauses autoplay. Studio controls expose three GSAP experiments in the normal carousel, paired SMALL menu, and ultra-minimal menu: slide the old category out to the left while the new category enters from the right, fade between categories, or switch animation off. A measured, persistent carousel viewport prevents slide and fade transforms from changing the document scroll position. The active category is marked in the normal tab rail and counter, in the paired 01-07 controls, and in the three-group ultra-minimal tab rail.
