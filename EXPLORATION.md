...
The menu presentation can also switch from the image-led category carousel to an ultra-minimal menu study. That study uses deliberately tiny list typography, consolidates the tabs into Benedict + sandwiches, entrees + sauces, and drinks + desserts, and renders each group as two headed category columns plus one poster column. A separate checkbox places the current category inside the exact centered-pair SMALL constraints, with one text column and one image column.
